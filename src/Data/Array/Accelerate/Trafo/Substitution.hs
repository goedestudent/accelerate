...
compose2 f@(Lam lhsB (Body c)) g@(Lam lhsA (Lam lhsA' (Body b)))
  ...
  = Lam lhsA
    ...
    -- env :> env'1   env'1 :> env'2
    -- weakenWithLHS lhsA weakenWithLHS lhsA'
    -- lhsB env env'
    -- lhsB' env'2 a4
    -- lhsA' env'1 env'2
    -- lhsA env env'1
    -- sinkWithLHS lhsB lhsB' :: env :> env'2 -> env' :> a4
  -- = Stats.substitution "compose" . Lam lhs2 . Body $ substitute' f g
compose2 _
...
rebuildPreOpenAcc k av acc =
  case acc of
    ...
    PermutingExpand tp sz get a f1 a1
                              -> PermutingExpand tp <$> rebuildFun (pure . IE) av' sz <*> rebuildFun (pure . IE) av' get <*> k av a <*> rebuildFun (pure . IE) av' f1 <*> k av a1
