...
fuseExpandFun config fun = case fun of
    ...
    Alam lhs b -> Alam lhs (fuseExpandFun config b)

fuseExpand :: Config -> OpenAcc aenv arrs -> OpenAcc aenv arrs
fuseExpand config (OpenAcc pacc)
    -- Only run this pass when array fusion is turned on
  | array_fusion `member` options config = OpenAcc $ case pacc of

    -- Special cases for fusion with expand
    Permute f d a -> case fuseExpand config a of
      OpenAcc (Expand (TupRpair _ e) s sz get arr) -> PermutedExpand e s sz get arr f d
      a' -> Permute f (fuseExpand config d) a'

    Map tp f a -> case fuseExpand config a of
      OpenAcc (Expand _ s sz get arr) -> Expand tp s sz (f `compose2` get) arr
      a' -> Map tp f a'


    -- Continuation of AST traversal
    Alet lhs bnd body         -> Alet lhs (fuseExpand config bnd) (fuseExpand config body)
    Avar var                  -> Avar var
    Apair as bs               -> Apair (fuseExpand config as) (fuseExpand config bs)
    Anil                      -> Anil
    Atrace msg as bs          -> Atrace msg (fuseExpand config as) (fuseExpand config bs)
    Apply repr afun acc       -> Apply repr (fuseExpandFun config afun) (fuseExpand config acc)
    Aforeign repr asm afun a  -> Aforeign repr asm afun (fuseExpand config a)
    Acond p a1 a2             -> Acond p (fuseExpand config a1) (fuseExpand config a2)
    Awhile p f a              -> Awhile (fuseExpandFun config p) (fuseExpandFun config f) (fuseExpand config a)
    Use repr arr              -> Use repr arr
    Unit tp x                 -> Unit tp x
    Reshape shr sh a          -> Reshape shr sh (fuseExpand config a)
    Generate repr sh f        -> Generate repr sh f
    Transform repr sh p f a   -> Transform repr sh p f (fuseExpand config a)
    Replicate slice sh a      -> Replicate slice sh (fuseExpand config a)
    Slice slice a sh          -> Slice slice (fuseExpand config a) sh
    ZipWith tp f a1 a2        -> ZipWith tp f (fuseExpand config a1) (fuseExpand config a2)
    Fold f z a                -> Fold f z (fuseExpand config a)
    FoldSeg i f z a s         -> FoldSeg i f z (fuseExpand config a)  s
    Scan d f z a              -> Scan d f z (fuseExpand config a)
    Scan' d f z a             -> Scan' d f z (fuseExpand config a)
    Backpermute shr sh f a    -> Backpermute shr sh f (fuseExpand config a)
    Expand tp s sz get a      -> Expand tp s sz get (fuseExpand config a)
    PermutedExpand tp s sz get a f d
                              -> PermutedExpand tp s sz get (fuseExpand config a) f (fuseExpand config d)
    Stencil sr tp f b a       -> Stencil sr tp f b (fuseExpand config a)
    Stencil2 sr1 sr2 tp f b1 a1 b2 a2 
                              -> Stencil2 sr1 sr2 tp f b1 (fuseExpand config a1) b2 (fuseExpand config a2)
  | otherwise = OpenAcc pacc
...
manifest config (OpenAcc pacc) =
  ...
  Manifest $ case pacc of
    ...
    PermutedExpand t s sz get a f def
                            -> PermutedExpand t s sz get (delayed config a) f (manifest config def)
    ...
    -- ------------
    --
    -- Not truly a consumer, but also not purely a producer
    Expand t s sz get a     -> Expand t s sz get (delayed config a)
...
embedPreOpenAcc config matchAcc embedAcc elimAcc pacc
  ...
  $ case pacc of
    ...
    PermutedExpand t s sz get a f dfts
                                -> done $ PermutedExpand t s sz get (cvtA a) (cvtF f) (cvtA dfts)
    ...
    -- 
    -- Not truly a consumer, but also not purely a producer
    Expand t s sz get a -> done $ Expand t s sz get (cvtA a)
  ...
  where
    ...
    replaceA sh' f' avar pacc =
      case pacc of
        ...
        Expand tR s sz get a    -> Expand tR s (cvtF sz) (cvtF get) (cvtA a)
        PermutedExpand tR s sz get a f d
                                -> PermutedExpand tR s (cvtF sz) (cvtF get) (cvtA a) (cvtF f) (cvtA d)
