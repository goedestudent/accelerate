...
fuseExpand (OpenAcc pacc) = OpenAcc $ case pacc of
    -- Special cases for fusion with expand
    Permute f d a -> case fuseExpand a of
      OpenAcc (Expand (TupRpair _ e) sz get arr) -> PermutedExpand e sz get arr f d
    ...
    PermutedExpand tp sz get a f d
                              -> PermutedExpand tp sz get (fuseExpand a) f (fuseExpand d)
    Stencil sr tp f b a       -> Stencil sr tp f b (fuseExpand a)
    Stencil2 sr1 sr2 tp f b1 a1 b2 a2 
                              -> Stencil2 sr1 sr2 tp f b1 (fuseExpand a1) b2 (fuseExpand a2)
...
manifest config (OpenAcc pacc) =
  ...
  Manifest $ case pacc of
    ...
    PermutedExpand t sz get a f def
                            -> PermutedExpand t sz get (delayed config a) f (manifest config def)
    ...
    -- ------------
    --
    -- Not truly a consumer, but also purely a producer
  ...
  where
    ...
    replaceA sh' f' avar pacc =
      case pacc of
        ...
        Expand tR sz get a      -> Expand tR (cvtF sz) (cvtF get) (cvtA a)
        PermutedExpand tR sz get a f d
                                -> PermutedExpand tR (cvtF sz) (cvtF get) (cvtA a) (cvtF f) (cvtA d)
